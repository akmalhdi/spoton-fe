  ...
  return (
    <div className="absolute bg-white right-0 top-12 shadow-xl shadow-black/10 border border-gray-200 w-90">
      ...
      {cartList.map((item, index) => (
        <div className="border-b border-gray-200 p-4 flex gap-3">
          <div className="bg-primary-light aspect-square w-16 flex justify-center items-center">
            <Image className="aspect-square object-contain" src={`/images/products/${item.imgUrl}`} width={63} height={63} alt={item.name} />
          </div>
      ...
      <div className="border-t border-gray-200 p-4">
        ...
        <Button variant="dark" size="small" className="w-full mt-5">Checkout Now <FiArrowRight/></Button>
      </div>
    </div>
  );
};
