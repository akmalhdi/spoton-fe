...
const ProductsSection = () => {
  return (
    <section id="product-section" className="container mx-auto mt-32 pb-20">
        ...
        {productList.map((product, index) => (
          <Link href={`/product/${product.name}`} key={index} className="p-1.5 bg-white hover:drop-shadow-xl duration-300">
            <div className="bg-primary-light aspect-square w-full flex justify-center items-center relative">
              <Image src={`/images/products/${product.imgUrl}`} alt={product.name} width={250} height={250} className="aspect-square object-contain" />
            ...
            <div className="flex justify-between mb-8">
              ...
              <div className="text-primary font-medium">{priceFormatter(product.price)}</div>
            </div>
          </Link>
        ))}
      </div>
