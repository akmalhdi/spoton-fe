        ...
        {categoryList.map((category, index) => (
          <div className="rounded-lg bg-gradient-to-r from-[#F1F1F1] to-[#F7F7F7] w-full aspect-square flex justify-center" key={index}>
            <div className="self-center">
              <Image className="mb-[10px]" src={`/images/categories/${category.imgUrl}`} width={86} height={86} alt={category.name} />
              <div className="text-primary text-medium text-xl text-center">{category.name}</div>
            </div>
          </div>
        ))}
      </div>
