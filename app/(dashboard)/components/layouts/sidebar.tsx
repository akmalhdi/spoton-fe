    ...
    <aside className="w-80 min-h-screen bg-white border-r border-gray-100 flex flex-col fixed left-0 top-0">
      ...
      <div className="flex flex-col gap-2 mt-12 px-5">
        {menuItems.map((item, index) => {
          ...
          return (
            <Link className={`flex gap-3 py-3 px-4.5 items-center rounded-lg font-medium duration-300 ${isActive ? "bg-primary/15 text-primary" : "hover:bg-gray-100"}`} href={item.link} key={index}>
              <item.icon size={24} />
              <span>{item.name}</span>
            </Link>
          );
      ...
      <button onClick={handleLogout} className="flex cursor-pointer gap-3 font-medium mx-5 py-5 px-4.5 mb-5 hover:bg-gray-100 duration-300 rounded-lg items-center mt-auto">
        <FiLogOut />
        Log out
      </button>
    </aside>
